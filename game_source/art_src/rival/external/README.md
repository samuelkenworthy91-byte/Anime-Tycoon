...
The GitHub workflow `.github/workflows/finalize-rival-posters.yml` is scoped to `work/final-awards-rival-posters` as the reusable staging/finalisation branch. It runs the import, test suite, production build, and Cast V3 coverage audit whenever the numbered source posters/catalog/finaliser change there. Runtime assets generated by that workflow are committed to `main` after validation.

...
const releaseOffsets = [1, 4, 7, 11] as const;
    ...
    <>
      <div className="rounded-xl border border-cyanx/30 bg-abyss/55 p-3">
        ...
        <div className="mt-2 grid grid-cols-3 gap-1.5 text-center">
          <div className="rounded-lg border border-line bg-panel2/55 p-1.5"><b className="block text-sm text-paper">{quarterActive.length}</b><span className="text-[8px] text-paper/40">ACTIVE NOW</span></div>
          <div className="rounded-lg border border-line bg-panel2/55 p-1.5"><b className="block text-sm text-gold">{plans.length}</b><span className="text-[8px] text-paper/40">PLANNED Q{quarter + 1}</span></div>
          <div className={cn("rounded-lg border p-1.5", warnings.some((warning) => warning.severity === "danger") ? "border-neon/45 bg-neon/8" : "border-mint/35 bg-mint/5")}><b className={cn("block text-sm", warnings.some((warning) => warning.severity === "danger") ? "text-neon" : "text-mint")}>{warnings.length}</b><span className="text-[8px] text-paper/40">PLANNING FLAGS</span></div>
        </div>
        ...
        <div className="fixed inset-0 z-[120] flex flex-col bg-[#060713] text-paper">
          <div className="flex items-center gap-2 border-b border-line bg-ink/95 px-3 py-3 shadow-lg">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="text-[9px] text-paper/45">Pick a release window. The studio estimates everything backwards for you.</div>
            </div>
          ...
          <div className="nice-scroll flex-1 overflow-y-auto px-2.5 py-3 sm:px-4">
            <div className="mx-auto max-w-5xl space-y-3">
              <div className="rounded-xl border border-line bg-panel2/55 p-1.5">
                <div className="flex items-center gap-1">
                  <button aria-label="Previous quarter" onClick={() => shiftQuarter(-1)} className="btn-press flex h-11 w-11 shrink-0 items-center justify-center rounded-lg text-paper/55"><ChevronLeft size={17}/></button>
                  <div className="min-w-0 flex-1 text-center">
                    <div className="text-[9px] font-black tracking-[0.16em] text-paper/40">YEAR {year}</div>
                    <div className="font-display text-sm font-extrabold text-cyanx">Q{quarter + 1} · {dateLabel(quarterStart)}–{dateLabel(quarterEnd)}</div>
                  </div>
                  <button aria-label="Next quarter" onClick={() => shiftQuarter(1)} className="btn-press flex h-11 w-11 shrink-0 items-center justify-center rounded-lg text-paper/55"><ChevronRight size={17}/></button>
                </div>
              ...
              {planning ? (
                <div className="rounded-xl border border-cyanx/40 bg-cyanx/5 p-3">
                  <div className="flex items-center justify-between gap-2"><div><div className="font-display text-sm font-extrabold text-cyanx">{editingPlanId ? "EDIT PLAN" : "PLAN A SHOW"}</div><div className="text-[9px] text-paper/45">Four choices. The calendar does the scheduling maths.</div></div><button onClick={resetPlanner} className="flex h-10 w-10 items-center justify-center rounded-lg text-paper/45"><X size={15}/></button></div>

                  <div className="mt-3 text-[9px] font-black tracking-wider text-paper/40">1 · WHAT KIND?</div>
                  <div className="mt-1 grid grid-cols-3 gap-1">{(["original", "franchise", "licensed"] as SlatePlanKind[]).map((id) => <button key={id} onClick={() => { setKind(id); setProperty(""); }} className={cn("min-h-11 rounded-lg border px-1.5 text-[9px] font-black", kind === id ? "border-cyanx bg-cyanx/15 text-cyanx" : "border-line text-paper/45")}>{id.toUpperCase()}</button>)}</div>

                  {kind === "original" && <><div className="mt-3 text-[9px] font-black tracking-wider text-paper/40">2 · ROUGH GENRE <span className="font-normal">(optional)</span></div><select value={genre} onChange={(event) => setGenre(event.target.value as GenreId | "")} className="mt-1 min-h-11 w-full rounded-lg border border-line bg-panel2 px-2 text-xs"><option value="">Decide later</option>{unlockedGenres.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></>}
                  {kind === "franchise" && <><div className="mt-3 text-[9px] font-black tracking-wider text-paper/40">2 · WHICH FRANCHISE?</div><select value={property} onChange={(event) => setProperty(event.target.value)} className="mt-1 min-h-11 w-full rounded-lg border border-line bg-panel2 px-2 text-xs"><option value="">Choose franchise…</option>{franchises.map((item) => <option key={item.key} value={item.key}>{item.baseTitle}</option>)}</select></>}
                  {kind === "licensed" && <><div className="mt-3 text-[9px] font-black tracking-wider text-paper/40">2 · WHICH OWNED IP?</div><select value={property} onChange={(event) => setProperty(event.target.value)} className="mt-1 min-h-11 w-full rounded-lg border border-line bg-panel2 px-2 text-xs"><option value="">Choose owned IP…</option>{ownedIps.map(({ contract, ip }) => <option key={contract.ipId} value={contract.ipId}>{ip!.title}</option>)}</select></>}

                  <div className="mt-3 text-[9px] font-black tracking-wider text-paper/40">3 · HOW IMPORTANT?</div>
                  <div className="mt-1 grid grid-cols-3 gap-1">{(["supporting", "standard", "tentpole"] as SlateImportance[]).map((id) => <button key={id} onClick={() => setImportance(id)} className={cn("min-h-11 rounded-lg border px-1.5 text-[9px] font-black", importance === id ? "border-gold bg-gold/10 text-gold" : "border-line text-paper/45")}>{importanceLabel[id]}</button>)}</div>

                  <div className="mt-3 text-[9px] font-black tracking-wider text-paper/40">4 · WHEN SHOULD IT RELEASE?</div>
                  <div className="mt-1 grid grid-cols-4 gap-1">{releaseOffsets.map((offset, index) => <button key={offset} onClick={() => setReleaseOffset(offset)} className={cn("min-h-11 rounded-lg border px-1.5 text-[9px] font-black", releaseOffset === offset ? "border-mint bg-mint/10 text-mint" : "border-line text-paper/45")}><span className="block">{releaseOffsetLabel[index]}</span><span className="block text-[8px] font-normal opacity-60">W{offset + 1}</span></button>)}</div>

                  <button disabled={kind !== "original" && !property} onClick={addPlan} className="btn-press mt-3 min-h-12 w-full rounded-xl border border-cyanx/55 bg-cyanx/12 text-[10px] font-black text-cyanx disabled:opacity-30"><Check size={13} className="mr-1 inline"/>{editingPlanId ? "SAVE CHANGES" : "ADD TO CALENDAR"}</button>
                </div>
              ) : (
              ...
              <div className="grid grid-cols-4 gap-1 sm:grid-cols-7">
                {stageLegend.map((item) => <div key={item.stage} className={cn("rounded-lg border px-1.5 py-1 text-center text-[8px] font-black", stageStyle[item.stage])}>{item.label.toUpperCase()}</div>)}
              </div>
                ...
                <div className="space-y-3">
                  {quarterActive.map((project) => (
                    <div key={project.id}>
                      ...
                      <CalendarStrip weeks={weeks} stageForWeek={(week) => projectCalendarStage(project, week)} />
                    </div>
                  ))}

...
describe("staff story chains",()=>{
 ...
 it("follows a showcase with an eligible mentorship offer",()=>{let r=base();r={...r,expansion:{...expansionOf(r),stories:[{...offered(),status:"resolved",decision:"celebrate",nextDay:14}]},day:14};const next=advanceStaffStories(r);expect(expansionOf(next).stories!.at(-1)?.kind).toBe("mentorship");});

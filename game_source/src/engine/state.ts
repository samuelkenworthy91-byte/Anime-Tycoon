...
export interface RunState {
  ...
  staff: Staff[];
  candidates: Staff[];
  /** recruitment-ad refreshes bought in the stored absolute month */
...
const recruitmentMonth = (week: number) => Math.floor(Math.max(0, week) / 4);

/** The first advert each month is £8k, then each further advert costs £4k more. */
export function recruitmentAdCost(r: RunState): number {
  const refreshes = r.recruitmentAdMonth === recruitmentMonth(r.week) ? r.recruitmentAdRefreshes : 0;
  return RECRUITMENT_AD_BASE_COST + refreshes * RECRUITMENT_AD_COST_STEP;
}

export function refreshRecruitmentAds(r: RunState): RunState | null {
  const month = recruitmentMonth(r.week);
  const refreshes = r.recruitmentAdMonth === month ? r.recruitmentAdRefreshes : 0;
  const cost = RECRUITMENT_AD_BASE_COST + refreshes * RECRUITMENT_AD_COST_STEP;
  if (r.cash < cost) return null;
  return {
    ...r,
    cash: r.cash - cost,
...
export function advanceWeeks(r: RunState, n: number, opts: { liveDaysAlreadyApplied?: boolean } = {}): RunState {
  ...
  for (let i = 1; i <= n; i++) {
    ...
    /* Archive & Research Room pays in visible fortnightly batches: 5 / 10 / 15 RD. */
    rd += fx.rdWeekly;
    const archiveTier = r.facilities.archive ?? 0;
    if (archiveTier > 0 && w % 2 === 0) rd += [5, 10, 15][archiveTier - 1];
...
export function applyResearchCompletion<T extends ResearchCarrier>(
  ...
  if (researchId === "genre_studies") {
    ...
    const discoveries = unknown.slice(0, progressiveResearchBatch(knownBefore));
...
export const RESEARCH_DISCOVERY_START = 6;
export const RESEARCH_DISCOVERY_STEP = 4;
export const RESEARCH_DISCOVERY_CAP = 30;

function progressiveResearchBatch(known: number): number {
  let completed = 0;
  let covered = 0;
  while (covered < Math.max(0, known) && completed < 20) {
    covered += Math.min(RESEARCH_DISCOVERY_CAP, RESEARCH_DISCOVERY_START + completed * RESEARCH_DISCOVERY_STEP);
    completed += 1;
  }
  return Math.min(RESEARCH_DISCOVERY_CAP, RESEARCH_DISCOVERY_START + completed * RESEARCH_DISCOVERY_STEP);
}

function progressiveResearchPasses(known: number): number {
  let completed = 0;
  let covered = 0;
  while (covered < Math.max(0, known) && completed < 20) {
    covered += Math.min(RESEARCH_DISCOVERY_CAP, RESEARCH_DISCOVERY_START + completed * RESEARCH_DISCOVERY_STEP);
...
function repeatResearchRunIndex(r: Pick<RunState, "research" | "arcGenreKnowledge" | "arcCombos">, id: string): number {
  ...
  if (id === "genre_studies") {
    const known = ARC_RESEARCH_ALL_GENRE_KEYS.filter((key) => (r.arcGenreKnowledge?.[key] ?? 0) > 0).length;
    return progressiveResearchPasses(known);
  }
  if (id === "narrative_analytics") {
    const known = ARC_RESEARCH_ALL_COMBO_IDS.filter((comboId) => r.arcCombos.includes(comboId)).length;
    return progressiveResearchPasses(known);

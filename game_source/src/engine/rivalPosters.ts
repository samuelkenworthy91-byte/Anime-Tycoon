...
/** Dedicated Year-5 Big Three flagship art; never consumed by routine rival slates. */
 ...
 * eligibility, so the player can deliberately choose unusual art. */
export function genericPosterOptions(
  animeType: AnimeType,
  genres: GenreId[],
  count = RIVAL_POSTERS.length,
  blocked: readonly string[] = [],
): RivalPoster[] {
  const fit = (p: RivalPoster) => {
    let score = p.animeTypes.includes(animeType) ? 5 : 0;
  ...
  const eligible = RIVAL_POSTERS
    .filter((p) => !BIG_THREE_RESERVED_POSTER_IDS.has(p.id) && !blockedIds.has(p.id))

...
def patch_once(text: str, old: str, new: str, label: str) -> str:
    ...
    if count == 0 and new in text:
        return text
    if count != 1:
        raise SystemExit(f"{label}: expected one patch point, found {count}")
    return text.replace(old, new, 1)


# ---------------------------------------------------------------------------
# 1) Make the integration patch itself robust: the old fan-effect insertion
# matched two unrelated `const scope = ...` lines. Anchor it to previewResult.
# ---------------------------------------------------------------------------
s = APPLY.read_text()
old_fan = '''s = once(s,
...
'    castAffinityDiscovered: r.castAffinityDiscovered,\\n  });\\n  const decisionFanMult = decisionReleaseFansMult(r, d);\\n  if (Math.abs(decisionFanMult - 1) > 0.001) {\\n    out = {\\n      ...out,\\n      fans: Math.round(out.fans * decisionFanMult),\\n      breakdown: [...out.breakdown, { label: \\"Decision-event audience effect\\", pts: `×${decisionFanMult.toFixed(2)} fans` }],\\n    };\\n  }\\n  const scope = PRODUCTION_SCOPES[d.scope ?? \\"standard\\"];\\n',
"decision fan mult")'''
s = patch_once(s, old_fan, new_fan, "preview fan anchor")

# ---------------------------------------------------------------------------
# 2) Add exact audience-learning support to the patch. Three DISTINCT tested
# releases of the same exact combo unlock the exact combo target plus the exact
# per-genre targets in the Direction Meeting tooltip. Repeating six panels on
# one show still counts as one series.
# ---------------------------------------------------------------------------
marker = '# --------------------------------------------------------------- App.tsx\n'
...
# ---------------------------------------------------------------------------
# 3) Curated single-genre targets. These are explicit rather than hash-jittered
# so every individual genre is explainable and intentionally separated. Pair
# targets still get a deterministic ±13 point fingerprint around the two
# singles' midpoint; a CI test confirms all 253 pair targets remain unique.
# ---------------------------------------------------------------------------
...
/**
 * Exact direction fingerprints for a single genre.
 * Axes are the live UI axes, always % toward the FIRST label:
 *   1 Plot ↔ Characters
 *   2 Sakuga ↔ Consistency
 *   3 Soundtrack ↔ Voice Cast
 *
 * These are hand-authored to be meaningfully distinct and genre-sensible.
 */
...
export function genreTargetFor(genres: GenreId[]): GenreProductionTarget {
  ...
  if (ids.length === 1) {
    return {
      key,
      ideal: [...SINGLE_GENRE_DIRECTION[ids[0]]] as [number, number, number],
      ratio: [...defs[0].ratio] as [number, number, number],
      comboQualityMult: 1,
    };
  }
  ...
  /* Each exact pair has a much more idiosyncratic fingerprint than either
     constituent genre. ±13 is enough to make the pair genuinely learnable
     without producing nonsensical extremes. */
  const ideal = baseIdeal.map((v, i) => {
    const jitter = (unit(seed, i + 1) * 2 - 1) * 13;
...
print("decision hardmode patch hardened; curated genre targets staged")
